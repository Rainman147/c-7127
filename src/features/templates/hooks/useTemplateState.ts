...
export const useTemplateState = ({ 
  ...
  console.log('[useTemplateState] Initializing with:', { 
    currentChatId, 
    initialTemplateId,
    selectedTemplateId: selectedTemplate.id 
  });

  // Load template from database or URL
  useEffect(() => {
    const loadTemplate = async () => {
      if (!currentChatId && !initialTemplateId) {
        console.log('[useTemplateState] No template to load, using default');
      ...
      try {
        ...
        if (initialTemplateId) {
          ...
          if (template) {
            console.log('[useTemplateState] Loading template from URL:', template.name);
        ...
        if (currentChatId) {
          ...
          if (templateType) {
            ...
            if (template) {
              console.log('[useTemplateState] Loaded template from database:', template.name);
              setSelectedTemplate(template);
              onTemplateChange(template);
            }
          }
        ...
        console.error('[useTemplateState] Error loading template:', error);
        toast({
          title: "Error",
  ...
  const handleTemplateChange = useCallback(async (template: Template) => {
    console.log('[useTemplateState] Template change requested:', template.name);
    ...
    try {
      ...
      console.error('[useTemplateState] Failed to update template:', error);
      toast({
        title: "Error",
